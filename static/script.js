...
function applyFilters() {
  // fixed: was named mismatch (HTML called search(), JS had applyFilters())
...
// Load available zones from the server and populate the zone <select> elements
function loadZones() {
  fetch("/analytics/zones")
    .then((r) => r.json())
    .then((list) => {
      if (!Array.isArray(list)) return;
      const from = document.getElementById("from-zone");
      const to = document.getElementById("to-zone");

      // remove any non-placeholder options (keep the first option)
      while (from.options.length > 1) from.remove(1);
      while (to.options.length > 1) to.remove(1);
...
function updateCharts(data) {
  ...
  const chartDefaults = {
    ...
    scales: {
      x: {
        ticks: { font: { family: "DM Mono" }, color: "#666" },
        grid: { color: "#f0f0f0" },
      },
      y: {
        ticks: { font: { family: "DM Mono" }, color: "#666" },
...
document.addEventListener("DOMContentLoaded", () => {
  // populate dynamic lists, then load data
  loadZones();
  applyFilters();
});
